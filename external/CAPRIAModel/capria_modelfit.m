function capria_modelfit(fpath, prot_type, isprocessed, thresh)
    ...
    if isfile('mask_clusters_bin.nii.gz')
        mask_path = 'mask_clusters_bin.nii.gz';
    ...
    [status,cmdout] = system(['fslinfo ' data_path]); cmdouts = strsplit(cmdout,'\s','DelimiterType','RegularExpression');
    ...
    fabcmd = [fabcmd ' --data=' data_path ' --mask=' mask_path];
    ...
    tosystem('fslcpgeom fabber_out_latest/mean_disp2 fabber_out_latest/mean_disp_s -d')
    tosystem('fslcpgeom fabber_out_latest/mean_disp2 fabber_out_latest/mean_disp_p -d')
end