function [u] = fista(xfm, d, W, lambda, im_size, niter, step)
...
for iter = 1:niter
    ...
    %   cycle spin
    shift = [randi(im_size(1)./(2^W.level)), randi(im_size(2)./(2^W.level)), randi(im_size(3)./(2^W.level))];
    u   =   cyclespin(u, W, shift);
...
function y = cyclespin(x, W, shift)
    % x is vector flattenned from wavelet transform coefficients
    y = W * (circshift(W'*x, shift));
end
