...
do
    ...
    # 4. gridding reconstruction stage1 for scan_1 and scan_2
    if [[ ${step_ind} -eq 4 ]]; then
        for ind in ${inds[@]}
        do
            # rm ${perf_recon_path}/scan_${ind}/gridding_script
            subfolder="scan_${ind}"
            # paramfname="${code_path}/example_scripts/subspace_param_stage1_gridding.m"            
            # SUBCMD="cd('${code_path}');p.date='${date}';p.ind=${ind};p.shot_ind=1;sim_invivo_motion('${paramfname}',p)"
            # echo $MATLABCMD \"$SUBCMD\" > ${perf_recon_path}/${subfolder}/gridding_script
            # CMD="${FSLSUBCMD} -q short.q -s openmp,10 -t ${perf_recon_path}/${subfolder}/gridding_script"
            # ID=$(eval $CMD)

            SUBCMD="mcflirt -in ${perf_recon_path}/${subfolder}/subspace_stage1_gridding_1.nii.gz -out ${perf_recon_path}/${subfolder}/subspace_stage1_gridding_mcf -refvol 0 -plots -mats"
    ...
    if [[ ${step_ind} -eq 8 ]]; then
        ...
        do
            ...
            paramfname="${code_path}/example_scripts/anat_angi_param_stage1_gridding.m"
            mcf_mat="${perf_recon_path}/scan_${ind}/subspace_stage1_gridding_mcf.mat"
            SUBCMD="cd('${code_path}');include_path();p.date='${date}';p.ind=${ind};p.mcf_mat='${mcf_mat}';sim_invivo_motion('${paramfname}',p)"
            echo $MATLABCMD \"$SUBCMD\" > ${recon_path}/scan_${ind}/recon_anat_script1_gridding
            CMD="$FSLSUBCMD -q short.q -s openmp,8 -t ${recon_path}/scan_${ind}/recon_anat_script1_gridding"
            ID=$(eval $CMD)
        done
    fi
    ...
    if [[ ${step_ind} -eq 9 ]]; then
        ...
        do
            ...
            # CMD="$FSLSUBCMD -q short.q $SUBCMD"
            # ID=$(eval $CMD)
            SUBCMD="flirt -in ${recon_path}/scan_${ind}/anat1_gridding -ref ${recon_path}/scan_${static_ind}/anat2 -out ${recon_path}/scan_${ind}/anat1_gridding_reg -omat ${recon_path}/scan_${ind}/anat1_gridding_regxfm -dof 6"
            CMD="$FSLSUBCMD -q short.q $SUBCMD"
            ID=$(eval $CMD)
        done
    fi
    ...
    if [[ ${step_ind} -eq 10 ]]; then
        for ind in ${inds[@]}
        do
            angi_dir="${recon_path}/scan_${ind}/"
            rm ${angi_dir}/combine_script
            inpath="${perf_recon_path}/scan_${ind}/subspace_stage1_gridding_mcf.mat"
            xfm2="${recon_path}/scan_${ind}/anat1_gridding_regxfm"
            angi_outpath="${recon_path}/scan_${ind}/subspace_stage3_gridding_mcf.mat"
    ...
    if [[ ${step_ind} -eq 11 ]]; then
        ...
        do
            ...
            paramfname="${code_path}/example_scripts/anat_angi_param_stage3_gridding.m"
            mcf_mat="${recon_path}/scan_${ind}/subspace_stage3_gridding_mcf.mat"
            SUBCMD="cd('${code_path}');include_path();p.date='${date}';p.ind=${ind};p.mcf_mat='${mcf_mat}';sim_invivo_motion('${paramfname}',p)"
    ...
    if [[ ${step_ind} -eq 12 ]]; then
        ...
        do
            ...
            # rm ${recon_path}/${subfolder}/recon_struct_script3
            ...
            # mcf_mat="${perf_recon_path}/scan_${ind}/subspace_stage1_gridding_mcf.mat"
            # paramfname="${code_path}/example_scripts/perfusion_param_stage1_gridding.m"
            ...
            mcf_mat="${recon_path}/scan_${ind}/subspace_stage3_gridding_mcf.mat"
            paramfname="${code_path}/example_scripts/angio_param_stage3_gridding.m"
            SUBCMD="cd('${code_path}');p.mcf_mat='${mcf_mat}';p.date='${date}';p.ind=${ind};sim_invivo_motion('${paramfname}',p)"
            # echo $MATLABCMD \"$SUBCMD\" >> ${recon_path}/${subfolder}/recon_angio_script3
            echo $SUBCMD
            ...
            mcf_mat="${recon_path}/scan_${ind}/subspace_stage3_gridding_mcf.mat"
            paramfname="${code_path}/example_scripts/struct_param_stage3_gridding.m"
            SUBCMD="cd('${code_path}');p.mcf_mat='${mcf_mat}';p.date='${date}';p.ind=${ind};sim_invivo_motion('${paramfname}',p)"
            # echo $MATLABCMD \"$SUBCMD\" >> ${recon_path}/${subfolder}/recon_struct_script3

            # CMD="$FSLSUBCMD -q verylong.q -s openmp,10 -t ${recon_path}/${subfolder}/recon_angio_script3"
            # ID=$(eval $CMD)

            # CMD="$FSLSUBCMD -q short.q -s openmp,8 -t ${recon_path}/${subfolder}/recon_struct_script3"
            # ID=$(eval $CMD)
