...
recon_path="/vols/Data/okell/qijia/test_moco/cone_data_144_WE_ref/"
perf_recon_path="/vols/Data/okell/qijia/test_moco/cone_data_144_WE_ref/"
mkdir -p ${recon_path}
...
if [[ ${step_ind} -eq 4 ]]; then
        subfolder=""
        paramfname="${perf_recon_path}/subspace_param_stage1.m"
        ...
        for i in {1..98}
        do
            SUBCMD="cd('${code_path}');p.shot_ind=${i};sim_invivo_motion('${paramfname}',p)"
...
if [[ ${step_ind} -eq 5 ]]; then
    bash ${code_path}/example_scripts/stage1.sh ${perf_recon_path}
fi
