function [x1, x2] = admm_mismatch(E1, dd1, E2, dd2, lambda, patch_size, im_size, niter, rho_0, logpath)
    ...
    for ii = 1:niter
        ...
        if mod(ii,20)==1 && ~isempty(logpath)
            rd = reshape(x(1:prod(im_size)) - x(prod(im_size)+1:end), [E1.Nd, E1.Nt]);
            save_avw(abs(rd), [char(logpath), '/rd_', num2str(ii)], 'd', [1,1,1]);
        end
    end
    ...
    function y=afun_cg(x)
        y1 = reshape(E1.mtimes2(reshape(x(1:prod(im_size)), im_size)),[],1) + rho/2*(x(1:prod(im_size))-x(prod(im_size)+1:end));
        y2 = reshape(E2.mtimes2(reshape(x(prod(im_size)+1:end), im_size)),[],1) - rho/2*(x(1:prod(im_size))-x(prod(im_size)+1:end));
        y = [y1; y2];
    end
